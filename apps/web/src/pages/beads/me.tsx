          ...
          <ul className="divide-y divide-line">
            {state.items.map((item) => {
              ...
              return (
                <li
                  key={item.id}
                  className="flex items-center gap-3 px-5 py-3 text-[16.5px] text-ink"
                >
                  <BeadDot
                    hex={c?.hex ?? "#cccccc"}
                  ...
                  <span className="text-muted text-[15px] tnum">
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-white border border-line rounded-lg p-5">
      <p className="text-[14px] uppercase tracking-wider text-muted mb-1">{label}</p>
...
function GrowthChart({ points }: { points: BeadPeriodTotal[] }) {
  ...
  return (
    <div className="bg-white border border-line rounded-lg p-5">
      <p className="text-[14px] uppercase tracking-wider text-muted mb-3">
        ...
        {points.map((p) => {
          ...
          return (
            <div key={p.period_id} className="flex flex-col items-center gap-1.5">
              ...
              <p className="text-[14px] text-ink tnum">{formatSGD(p.total_sgd)}</p>
            </div>
          );
        })}
      </div>
