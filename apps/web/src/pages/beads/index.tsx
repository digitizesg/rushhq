  ...
  return (
    <section className="mx-auto max-w-[1100px] px-4 sm:px-6 py-6 sm:py-10 space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-[12px] uppercase tracking-wider text-muted mb-1">Beads</p>
  ...
  return (
    <article className="bg-white border border-line rounded-lg overflow-hidden">
      <header className="flex items-center justify-between px-5 py-4 border-b border-line">
        <h2 className="text-[18px] font-semibold text-ink">{child.short_name}</h2>
        ...
        <div className="grid grid-cols-2 gap-2 pt-2">
          {activeChart && (
            <Link
              ...
              className="inline-flex items-center justify-center h-9 rounded-md border border-line text-[13.5px] text-ink hover:bg-soft transition-colors"
            >
              View chart
            </Link>
          )}
          <Link
            ...
            className="inline-flex items-center justify-center h-9 rounded-md bg-primary text-white text-[13.5px] font-medium hover:bg-primary-strong transition-colors"
          >
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <p className="text-[15px] text-ink font-medium tnum">{value}</p>
      </div>
    </div>
  );
}
...
function StatusPill({ status }: { status: BeadPeriodStatus }) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 h-6 rounded-full text-[12px] ${s.bg} ${s.text}`}>
      {s.label}
    </span>
  );
}
      ...
      {points.map((p) => {
        ...
        return (
          <div key={p.period_id} className="flex flex-col items-center gap-1">
            ...
            <p className="text-[10.5px] uppercase tracking-wider text-muted tnum">
              {formatPeriodLabel(p.period_start).slice(0, 3)}
            </p>
            <p className="text-[12px] text-ink tnum">
