    ...
    <section className="mx-auto max-w-[1100px] px-4 sm:px-6 py-6 sm:py-10 space-y-6">
      ...
      {months.length === 0 ? (
        ...
        <div className="bg-white border border-line rounded-lg overflow-x-auto">
          <table className="w-full text-[15.5px] min-w-[640px]">
            <thead className="bg-soft text-muted text-left text-[14px]">
              <tr>
...
function PastMonthPicker({ existingMonths, onPick, onCancelled }: PastMonthPickerProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {quickPicks.length > 0 && (
        <div>
          <p className="text-[14px] uppercase tracking-wider text-muted mb-2">Quick pick</p>
          <div className="flex flex-wrap gap-1.5">
            {quickPicks.map((q) => (
              <button
                ...
                onClick={() => setChosen(q)}
                className={[
                  "px-2.5 h-7 rounded-full border text-[14.5px] tnum",
                  chosen === q
