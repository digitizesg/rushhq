...
export default function MyStocksPage() {
  ...
  return (
    <section className="mx-auto max-w-[820px] px-4 sm:px-6 py-6 sm:py-10 space-y-6">
      <div>
        <p className="text-[12px] uppercase tracking-wider text-muted mb-1">Your investment</p>
        <h1 className="text-[28px] font-medium text-ink">Hi {member.short_name}</h1>
      </div>
      ...
      <div className="bg-white border border-line rounded-lg p-6 sm:p-8 text-center space-y-2">
        <p className="text-[12.5px] uppercase tracking-wider text-muted">You have</p>
      ...
      {more && (
        <div className="bg-white border border-line rounded-lg p-5 space-y-3">
          ...
          {s.contributions.length > 0 && (
            <>
              <p className="text-[12.5px] uppercase tracking-wider text-muted mt-3">Your contributions</p>
              <ul className="space-y-1">
                {s.contributions.map((c) => (
                  <li key={c.id} className="text-[13.5px] text-ink flex items-baseline justify-between gap-3">
          ...
          <p className="text-[13px] text-muted leading-relaxed pt-3 border-t border-line">
      ...
      <p className="text-[12.5px] text-muted text-center pt-2">
...
function BigChart({ points }: { points: PortfolioSnapshot[] }) {
  ...
  return (
    <div className="bg-white border border-line rounded-lg p-5">
      <p className="text-[12.5px] uppercase tracking-wider text-muted mb-2">Last {points.length} months</p>
