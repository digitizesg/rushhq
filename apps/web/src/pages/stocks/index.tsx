  ...
  return (
    <section className="mx-auto max-w-[1100px] px-4 sm:px-6 py-6 sm:py-10 space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2 text-[14.5px] text-muted">
          {lastUpdated ? (
            <>
      ...
      <div className="flex items-start gap-2 text-[14.5px] text-muted bg-soft border border-line rounded-md px-3 py-2">
      ...
      {readyTotal > 0 && (
        <div className="bg-emerald-soft/40 border border-emerald/30 rounded-lg p-5 sm:p-6 flex flex-wrap items-center justify-between gap-3">
          ...
          <Link
            to="/stocks/buy"
            className="inline-flex items-center justify-center h-10 px-4 rounded-md bg-primary text-white text-[16px] font-medium hover:bg-primary-strong transition-colors"
          >
      ...
      <section className="bg-white border border-line rounded-lg p-5 sm:p-6 space-y-4">
        <p className="text-[14.5px] uppercase tracking-wider text-muted">Quick actions</p>
        <div className="flex flex-wrap gap-2.5">
          <Link
            to="/stocks/buy"
            className="inline-flex items-center gap-2 h-11 px-4 rounded-md bg-primary text-white text-[16px] font-medium hover:bg-primary-strong transition-colors shadow-sm"
          >
...
function ActionButton({
  ...
  return (
    <Link
      to={to}
      className="inline-flex items-center gap-2 h-11 px-4 rounded-md bg-white border border-line text-ink text-[16px] font-medium hover:bg-soft hover:border-ink/15 transition-colors"
    >
...
function Stat({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div>
      <p className="text-[13.5px] uppercase tracking-wider text-muted mb-1">{label}</p>
      <p className="text-[22px] font-semibold text-ink tnum">{value}</p>
      {sub && <p className="text-[14px] text-muted mt-0.5 tnum">{sub}</p>}
    </div>
  );
}
...
function GainStat({ label, amount, pct }: { label: string; amount: number; pct?: number }) {
  ...
  return (
    <div>
      ...
      {pct !== undefined && pct !== 0 && (
        <p className={["text-[14px] mt-0.5 tnum", positive ? "text-emerald" : "text-danger"].join(" ")}>
...
function KidGainHeadline({
  ...
  return (
    <div className={["rounded-md border px-4 py-3", tone].join(" ")}>
      <p className="text-[17px] font-medium">{message}</p>
      {gainPct !== 0 && (
        <p className="text-[14.5px] mt-0.5 opacity-80 tnum">
  ...
  return (
    <article className="bg-white border border-line rounded-lg overflow-hidden">
      ...
      >
        ...
        {pendingCount > 0 && (
          <span className="inline-flex items-center px-2 h-6 rounded-full text-[14px] bg-amber-soft text-amber">
  ...
  return (
    <div className="bg-white border border-line rounded-lg p-5 sm:p-6 space-y-4">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <p className="text-[14.5px] uppercase tracking-wider text-muted">Portfolio over time</p>
